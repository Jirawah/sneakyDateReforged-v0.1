...
public class CorsConfig {
    ...
    @Bean
    public CorsConfigurationSource corsConfigurationSource() {
        // Ex: "http://localhost:4200,http://127.0.0.1:4200"  |  "*" pour tout autoriser (dev)
        String origins = System.getenv().getOrDefault("CORS_ALLOWED_ORIGINS", "*").trim();

        CorsConfiguration cfg = new CorsConfiguration();
        cfg.setAllowedMethods(List.of("GET","POST","PUT","PATCH","DELETE","OPTIONS"));
        cfg.setAllowedHeaders(List.of("*"));
        cfg.setExposedHeaders(List.of("X-Request-Id"));
        cfg.setAllowCredentials(true); // on garde les credentials

        // ⚠️ Avec allowCredentials=true, "*" n'est pas autorisé dans allowedOrigins.
        // Utiliser allowedOriginPatterns("*") OU une liste explicite.
        if ("*".equals(origins)) {
